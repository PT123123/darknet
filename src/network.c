...
void backward_network_gpu(network *netp)
{
    ...
    for(i = net.n-1; i >= 0; --i){
	    ...
	    //1-6成功把GPU里的复制出来到CPU了，
	    float* dst_pt=(float*)calloc(prev.batch*prev.outputs);
	    cudaMemcpy(dst_pt,prev.delta_gpu,prev.batch*prev.outputs,cudaMemcpyDeviceToHost);//将gpu里的每层的delta_gpu复制到dst_pt
	    for(int i=0;i<(prev.batch)*(prev.outputs);i++)
		    if(*(dst+i)>66666){
			    printf("%f\n",*(dst+i));
			    *(dst+i)=66666;
			     }//1-7不是很确定这个阈值应该设置成多少
	    cudaMemcpy(prev.delta_gpu,dst_pt,prev.batch*prev.outputs,cudaMemcpyHostToDevice);
